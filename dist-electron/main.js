  ...
  var stringify = function stringify2(object, prefix, generateArrayPrefix, commaRoundTrip, allowEmptyArrays, strictNullHandling, skipNulls, encodeDotInKeys, encoder, filter, sort, allowDots, serializeDate, format, formatter, encodeValuesOnly, charset2, sideChannel2) {
    ...
    if (obj === null) {
      if (strictNullHandling) {
        return formatter(encoder && !encodeValuesOnly ? encoder(prefix, defaults.encoder, charset2, "key", format) : prefix);
      }
      obj = "";
    }
...
function requireServeStatic() {
  ...
  function serveStatic$1(root, options) {
    ...
    return function serveStatic2(req, res, next) {
      ...
      var stream = send2(req, path2, opts);
...
function getBackupConfig(backupDir) {
...
function saveBackupConfig(backupDir, config) {
  const configPath = path.join(backupDir, "config.json");
  try {
...
function startServer() {
  ...
  expressApp.get("/api/files/list", (req, res) => {
    try {
      ...
      const files = fs.existsSync(targetPath) ? fs.readdirSync(targetPath).map((name) => ({
        name,
